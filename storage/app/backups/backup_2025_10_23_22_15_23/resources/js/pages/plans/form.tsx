...
export default function PlanForm({ plan, hasDefaultPlan = false, otherDefaultPlanExists = false, availableAddons = [] }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if ((formData.duration === 'monthly' || formData.duration === 'both') && (!formData.price || formData.price <= 0)) {
      alert(t('Monthly price must be greater than 0'));
      setProcessing(false);
      return;
    }

    if ((formData.duration === 'yearly' || formData.duration === 'both') && (!formData.yearly_price || formData.yearly_price <= 0)) {
