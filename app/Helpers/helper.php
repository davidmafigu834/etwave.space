        ...
        if (is_dir($framework_path)) {
            foreach (\File::allFiles($framework_path) as $file) {
                try {
                    ...
                    // Ignore files that cannot be stat'ed (deleted between scan and size retrieval)
                    continue;
                }
            }
        }
...
if (! function_exists('getPaymentMethodConfig')) {
    ...
    function getPaymentMethodConfig($method, $userId = null)
    {
        ...
        switch ($method) {
            ...
            case 'paynow':
                return [
                    'enabled' => isPaymentMethodEnabled('paynow', $userId),
                    'merchant_id' => $settings['paynow_merchant_id'] ?? null,
                ];
...
if (! function_exists('getEnabledPaymentMethods')) {
    ...
    {
        $methods = ['free', 'manually', 'stripe', 'paypal', 'razorpay', 'mercadopago', 'paystack', 'flutterwave', 'bank', 'paytabs', 'skrill', 'coingate', 'payfast', 'tap', 'xendit', 'paytr', 'mollie', 'toyyibpay', 'cashfree', 'iyzipay', 'benefit', 'ozow', 'easebuzz', 'khalti', 'authorizenet', 'fedapay', 'payhere', 'cinetpay', 'paymentwall', 'paynow'];
        $enabled = [];
...
if (! function_exists('validatePaymentMethodConfig')) {
     ...
     * @param array $config
            ...
            case 'paynow':
                if (empty($config['merchant_id'])) {
                    $errors[] = 'Paynow merchant ID is required';
                }
                if (empty($config['integration_key'])) {
                    $errors[] = 'Paynow integration key is required';
                }
                break;
