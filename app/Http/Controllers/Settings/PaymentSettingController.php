...
class PaymentSettingController extends Controller
{
    ...
    private function preparePaymentSettings(Request $request, array $validatedData): array
    {
        return [
            ...
            'is_paynow_enabled' => $request->boolean('is_paynow_enabled'),
    ...
    private function validateEnabledPaymentMethods(Request $request, array $validatedData): void
    {
        ...
        if ($request->boolean('is_paynow_enabled')) {
            $config = [
                'merchant_id' => $validatedData['paynow_merchant_id'],
                'integration_key' => $validatedData['paynow_integration_key'],
            ];
            $validation = validatePaymentMethodConfig('paynow', $config);
            if (!$validation['valid']) {
                $errors = array_merge($errors, $validation['errors']);
            }
        }
    ...
    private function filterSensitiveData(array $settings): array
    {
        ...
        $enabledKeys = [
            ...
            'is_ozow_enabled', 'is_cashfree_enabled', 'is_paynow_enabled'
        ];
