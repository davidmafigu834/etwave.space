...
class MediaController extends Controller
{
    ...
    private function getFullUrl($url)
    {
        ...
        $prefix = rtrim($request->getBaseUrl(), '/');
    ...
    public function batchStore(Request $request)
    {
        ...
        if ($uploadedFiles instanceof UploadedFile) {
            ...
            $uploadedFiles = iterator_to_array($uploadedFiles ?? []);
        }

        $uploadedFiles = array_values(array_filter($uploadedFiles));
        $storageCheck = $this->checkStorageLimit($uploadedFiles);
        if ($storageCheck) {
            return $storageCheck;
        }
        ...
        $validator = \Validator::make([
            ...
            'files.*.mimes' => __('Only specified file types are allowed: :type',[
                    'type' => isset($config['allowed_file_types']) && trim($config['allowed_file_types']) !== ''
