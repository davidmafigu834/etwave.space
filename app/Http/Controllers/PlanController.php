...
class PlanController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Admin view - show all plans
        $billingCycle = $request->input('billing_cycle', 'monthly');
        $dbPlans = Plan::all(); // Show all plans for admin
    ...
    {
        \Log::info('PlanController@store called', ['request_data' => $request->all()]);
        ...
        try {
            $validated = $request->validate([
                ...
                'price.required' => 'Monthly price is required',
                'price.min' => 'Monthly price must be greater than 0',
                'price.numeric' => 'Monthly price must be a valid number',
                'yearly_price.min' => 'Yearly price cannot be negative',
                'yearly_price.numeric' => 'Yearly price must be a valid number',
                'duration.required' => 'Plan type is required',
                'duration.in' => 'Plan type must be monthly, yearly, or both',
                'business.required' => 'Number of businesses is required',
                'business.integer' => 'Number of businesses must be a whole number',
                'business.min' => 'Number of businesses cannot be negative',
                'bio_links.required' => 'Number of bio links is required',
                'bio_links.integer' => 'Number of bio links must be a whole number',
                'bio_links.min' => 'Number of bio links cannot be negative',
                'max_users.required' => 'Maximum users is required',
                'max_users.integer' => 'Maximum users must be a whole number',
                'max_users.min' => 'Maximum users cannot be negative',
                'storage_limit.required' => 'Storage limit is required',
                'storage_limit.numeric' => 'Storage limit must be a valid number',
                'storage_limit.min' => 'Storage limit cannot be negative',
            ]);
        ...
        // Additional validation for yearly_price based on duration
        if (($validated['duration'] === 'yearly' || $validated['duration'] === 'both')) {
            if (!isset($validated['yearly_price']) || $validated['yearly_price'] === null || $validated['yearly_price'] <= 0) {
                \Log::error('Yearly price validation failed', [
                    'duration' => $validated['duration'],
                    'yearly_price' => $validated['yearly_price']
                ]);
                return back()->withErrors(['yearly_price' => 'Yearly price is required and must be greater than 0 for yearly/both plans']);
            }
        }
        ...
        // Only calculate yearly_price for plans that support yearly billing
        if (($validated['duration'] === 'yearly' || $validated['duration'] === 'both')) {
            ...
            // Ensure yearly price is greater than 0 for yearly/both plans
            if ($validated['yearly_price'] <= 0) {
                return back()->withErrors(['yearly_price' => 'Yearly price must be greater than 0']);
            }
        } else {
            // For monthly-only plans, set yearly_price to null
        ...
        \Log::info('About to create plan', ['final_data' => $validated]);
    ...
    private function companyPlansView(Request $request)
    {
        ...
        // Show all active plans for company users
