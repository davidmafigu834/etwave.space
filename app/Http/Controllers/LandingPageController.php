...
class LandingPageController extends Controller
{
    public function show(Request $request)
    {
        ...
        if ($business) {
            // Track visit for business (skip for authenticated users to avoid truncation error)
            if (!auth()->check()) {
        ...
        // Track general landing page visit (skip for authenticated users)
        if (!auth()->check()) {
    ...
    {
        // Track general waitlist page visit (skip for authenticated users)
        if (!auth()->check()) {
            \Shetabit\Visitor\Facade\Visitor::visit();
        }
