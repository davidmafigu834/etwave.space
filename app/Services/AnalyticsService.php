...
class AnalyticsService
{
    ...
    public function getVisitorOverview($businessIds, $dateRange)
    {
        ...
        return Cache::remember($cacheKey, 1800, function () use ($businessIds, $dateRange) {
            ...
            $engagement = DB::table('shetabit_visits')
                ->whereIn('business_id', $businessIds)
                ...
                ->selectRaw('SUM(JSON_LENGTH(request) - 1) as additional_page_views')
                ->first();

            $pageViews = (int) ($engagement->page_views ?? 0);
            $uniqueVisitors = (int) ($engagement->unique_visitors ?? 0);
            $additionalViews = (int) ($engagement->additional_page_views ?? 0);

            $totalSessions = max($uniqueVisitors, 1);
            $totalInteractions = max($pageViews + $additionalViews, 0);

            $avgSessionDuration = 0;
            $bounceRate = 0;

            if ($totalSessions > 0) {
                $estimatedDurationPerPage = 45; // seconds
                $avgSessionDuration = (int) round(($totalInteractions / $totalSessions) * $estimatedDurationPerPage);

                $singlePageSessions = max($pageViews - $additionalViews, 0);
    ...
    public function getDeviceAnalytics($businessIds, $dateRange)
    {
        ...
        $subQuery = DB::table('shetabit_visits')
            ->whereIn('business_id', $businessIds)
            ->whereBetween('created_at', $dateRange)
            ->selectRaw('
                CASE
        ...
        return DB::table(DB::raw("({$subQuery->toSql()}) as sub"))
            ...
            ->selectRaw('device_category as device, COUNT(*) as visitors')
