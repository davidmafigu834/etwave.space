...
use App\Http\Controllers\PaynowPaymentController;
...
// Update route - redirect to dashboard if logged in, or login page if not
...
// Paynow callback/success routes (public)
Route::match(['GET', 'POST'], 'payments/paynow/callback', [PaynowPaymentController::class, 'callback'])->name('paynow.callback');
Route::get('payments/paynow/success', [PaynowPaymentController::class, 'success'])->name('paynow.success');
...
Route::get('/refresh-language/{locale}', [TranslationController::class, 'refreshLanguage'])->name('refresh-language.v1');
...
Route::middleware(['auth', 'verified','setting'])->group(function () {
    ...
    Route::post('paynow/create-payment', [PaynowPaymentController::class, 'createPayment'])->name('paynow.create-payment');
    ...
    Route::delete('media/{id}', [MediaController::class, 'destroy'])->middleware('permission:delete-media')->name('media.destroy');
        ...
        // Plans routes - accessible without plan check for both admin and company users
        Route::get('plans', [PlanController::class, 'index'])->name('plans.index');
        Route::post('plans/request', [PlanController::class, 'requestPlan'])->name('plans.request');
        Route::post('plans/cancel-request', [PlanController::class, 'cancelRequest'])->name('plans.cancel-request');
        Route::post('plans/trial', [PlanController::class, 'startTrial'])->name('plans.trial');
...
Route::post('landing-page/subscribe', [LandingPageController::class, 'subscribe'])->name('landing-page.subscribe');
Route::get('waitlist', [LandingPageController::class, 'waitlist'])->name('waitlist');
Route::get('etwave', function () {
    return Inertia::render('etwave-landing');
})->name('etwave.landing');
...
Route::middleware('superadmin:manage-system-updates')->group(function () {
    Route::get('system-updates', [UpdateController::class, 'index'])->name('system-updates.index');
    Route::get('system-updates/create', [UpdateController::class, 'create'])->name('system-updates.create');
    Route::post('system-updates', [UpdateController::class, 'store'])->name('system-updates.store');
    Route::post('system-updates/{update}/apply', [UpdateController::class, 'apply'])->name('system-updates.apply');
    Route::post('system-updates/{update}/rollback', [UpdateController::class, 'rollback'])->name('system-updates.rollback');
    Route::delete('system-updates/{update}', [UpdateController::class, 'destroy'])->name('system-updates.destroy');
    Route::get('system-updates/{update}/download', [UpdateController::class, 'download'])->name('system-updates.download');
    Route::get('system-updates/{update}/status', [UpdateController::class, 'status'])->name('system-updates.status');
    Route::post('system-updates/{update}/logs', [UpdateController::class, 'logs'])->name('system-updates.logs');
    Route::post('system-updates/bulk-action', [UpdateController::class, 'bulkAction'])->name('system-updates.bulk-action');
    Route::post('system-updates/validate', [UpdateController::class, 'validateUpdate'])->name('system-updates.validate');
    Route::get('system-updates/check', [UpdateController::class, 'checkForUpdates'])->name('system-updates.check');
    Route::get('system-info', [UpdateController::class, 'systemInfo'])->name('system-updates.system-info');
});
...
// Root route - Default homepage (etwave landing page)
Route::get('/', function () {
    return Inertia::render('etwave-landing');
})->name('home');
