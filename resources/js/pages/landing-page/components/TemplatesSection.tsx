...
// Template name to key mapping for cases where they don't match exactly
const templateNameToKeyMap: Record<string, string> = {
...
export default function TemplatesSection({ settings, sectionData, brandColor }: TemplatesSectionProps) {
  ...
  const openPreview = (templateName: string) => {
    // Map template name to correct template key
    const templateKey = templateNameToKeyMap[templateName] || templateName;
    
    // Store the template data in localStorage for the preview page to use
    const templateData = getBusinessTemplate(templateKey);
    const defaultData = templateData?.defaultData || {};
    
    // Create proper config sections with enabled flags
    const configSections: any = {};
    
    // Copy all default data sections and ensure they're enabled
    Object.keys(defaultData).forEach(sectionKey => {
    ...
    // Ensure essential sections exist with content if missing
    if (!configSections.header) {
      configSections.header = {
        name: templateData?.defaultData?.header?.name || 'Business Name',
        title: 'Professional Services',
        tagline: 'Providing quality services with expertise',
        enabled: true
      };
    }
    
    if (!configSections.contact) {
    ...
    // Get allowed sections from template
    const allowedSections = templateData?.sections?.map(section => section.key) || [];
    
    localStorage.setItem('vcard_preview_data', JSON.stringify({
      business_type: templateKey,
      name: templateData?.defaultData?.header?.name || 'Business Name',
      slug: 'preview',
