...
export default function PaymentSettings({ settings = {} }: PaymentSettingsProps) {
  ...
  const { data, setData, post, processing, errors } = useForm<PaymentSettings>({
    ...
    is_paynow_enabled: settings.is_paynow_enabled === true || settings.is_paynow_enabled === '1',
    paynow_merchant_id: settings.paynow_merchant_id || '',
  ...
  const paymentMethods = useMemo(() => [
    ...
    { key: 'paynow', name: t(PAYMENT_METHOD_LABELS[PAYMENT_METHODS.PAYNOW]) },
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              {/* Paynow */}
              {shouldShowMethod('paynow') && (
              <PaymentMethodCard
                title={t(PAYMENT_METHOD_LABELS[PAYMENT_METHODS.PAYNOW])}
                icon={<CreditCard className="h-5 w-5" />}
                enabled={data.is_paynow_enabled}
                onToggle={(checked) => setData('is_paynow_enabled', checked)}
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <PaymentInputField
                    id="paynow_merchant_id"
                    label={t('Merchant ID')}
                    value={data.paynow_merchant_id}
                    onChange={(value) => setData('paynow_merchant_id', value)}
                    placeholder="e.g. 12345"
                    error={errors.paynow_merchant_id}
                  />
                  <PaymentInputField
                    id="paynow_integration_key"
                    label={t('Integration Key')}
                    value={data.paynow_integration_key}
                    onChange={(value) => setData('paynow_integration_key', value)}
                    placeholder="••••••••"
                    isSecret
                    error={errors.paynow_integration_key}
                  />
                </div>
                <p className="text-xs text-muted-foreground mt-2">
                  {t('Set your Paynow return URL to {{returnUrl}} and result URL to {{resultUrl}} in the Paynow dashboard.', {
                    returnUrl: window?.location ? `${window.location.origin}/payments/paynow/success` : 'https://your-domain.com/payments/paynow/success',
                    resultUrl: window?.location ? `${window.location.origin}/payments/paynow/callback` : 'https://your-domain.com/payments/paynow/callback'
                  })}
                </p>
              </PaymentMethodCard>
              )}
