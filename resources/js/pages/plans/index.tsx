...
export default function Plans({ plans: initialPlans, billingCycle: initialBillingCycle = 'monthly', hasDefaultPlan, isAdmin = false, currentPlan, userTrialUsed, paymentMethods = [] }: Props) {
  ...
  const handleSubscribe = async (planId: number) => {
    ...
    if (plan) {
      try {
        const response = await fetch(route('payment.methods', { user_id: currentPlan?.user_id || undefined }));
  ...
  const formatPaymentMethods = (paymentSettings: any) => {
    ...
    if (paymentSettings?.is_paynow_enabled === true || paymentSettings?.is_paynow_enabled === '1') {
      methods.push({
        id: 'paynow',
        name: t('Paynow (Zimbabwe)'),
