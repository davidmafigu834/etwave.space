...
export default function PlanForm({ plan, hasDefaultPlan = false, otherDefaultPlanExists = false, availableAddons = [] }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      alert(t('Plan name is required'));
      setProcessing(false);
      return;
    }

    if (!formData.duration) {
      alert(t('Plan type is required'));
      setProcessing(false);
      return;
    }

    if ((formData.duration === 'monthly' || formData.duration === 'both') && formData.price !== 0 && (formData.price <= 0 || !formData.price)) {
      alert(t('Monthly price must be greater than 0'));
      setProcessing(false);
      return;
    }

    if ((formData.duration === 'yearly' || formData.duration === 'both') && formData.yearly_price !== 0 && (formData.yearly_price <= 0 || !formData.yearly_price)) {
      alert(t('Yearly price is required and must be greater than 0 for yearly/both plans'));
      setProcessing(false);
      return;
    }

    // Transform form data to ensure proper types and structure
    const transformedData = {
      ...formData,
      price: parseFloat(formData.price) || 0,
      yearly_price: (formData.duration === 'yearly' || formData.duration === 'both') 
        ? (parseFloat(formData.yearly_price) || 0)
        : null, // Set to null for monthly-only plans
      business: parseInt(formData.business) || 0,
      bio_links: parseInt(formData.bio_links) || 0,
      max_users: parseInt(formData.max_users) || 0,
      storage_limit: parseFloat(formData.storage_limit) || 0,
      trial_day: formData.trial_day ? parseInt(formData.trial_day) : 0,
      // Ensure arrays are properly formatted
      themes: Array.isArray(formData.themes) ? formData.themes : [],
      bio_links_themes: Array.isArray(formData.bio_links_themes) ? formData.bio_links_themes : [],
      addons: Array.isArray(formData.addons) ? formData.addons : [],
      // Ensure features object is clean
      features: formData.features || {},
    };

    if (isEdit) {
      router.put(route('plans.update', plan.id), transformedData, {
        onFinish: () => setProcessing(false)
      });
    } else {
      router.post(route('plans.store'), transformedData, {
        onFinish: () => setProcessing(false)
      });
    }
  };
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const allSections = Object.keys({
                        ...
                        'whatsapp': t('WhatsApp Call-to-Action'),
