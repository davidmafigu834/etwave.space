...
export default function FreelancerTemplate({ data, template }: FreelancerTemplateProps) {
  ...
  const renderSection = (sectionKey: string) => {
    ...
    switch (sectionKey) {
      ...
      case 'whatsapp':
        return renderWhatsAppSection(sectionData);
      case 'qr_share':
  ...
  const renderWhatsAppSection = (whatsappData: any) => {
    if (!whatsappData.phone_number) return null;

    // Generate WhatsApp URL with prefilled message
    const whatsappUrl = `https://wa.me/${whatsappData.phone_number.replace(/\D/g, '')}?text=${encodeURIComponent(whatsappData.prefilled_message || '')}`;

    return (
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.borderColor || '#334155'}` }}>
        <h3 className="font-semibold text-sm mb-3" style={{ color: colors.text, fontFamily: font }}>{t('WhatsApp')}</h3>

        {whatsappData.cta_text && (
          <p className="text-xs mb-3" style={{ color: colors.text, fontFamily: font }}>
            {whatsappData.cta_text}
          </p>
        )}
        ...
        <Button
          ...
          onClick={() => typeof window !== "undefined" && window.open(whatsappUrl, '_blank', 'noopener,noreferrer')}
        >
          {whatsappData.show_icon !== false && (
          ...
          {whatsappData.button_label || t('Message on WhatsApp')}
        </Button>
      </div>
    );
  };
