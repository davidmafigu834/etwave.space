...
// Function to clear menu cache and refresh sidebar
export const forceRefreshSidebar = () => {
    clearGlobalMenuCache();
    // Trigger a custom event to force sidebar re-render
    window.dispatchEvent(new CustomEvent('sidebar-refresh'));
    // Also dispatch the menu refresh event
    window.dispatchEvent(new CustomEvent('force-menu-refresh'));
};
...
export const AppSidebar = React.memo(() => {
    ...
    const userRole = auth.user?.type || (auth.user?.roles?.includes('superadmin') ? 'superadmin' : auth.user?.roles?.includes('company') ? 'company' : auth.user?.role);
    ...
    useEffect(() => {        
        ...
        // Listen for force menu refresh events
        const handleForceMenuRefresh = () => {
            clearGlobalMenuCache();
        ...
        window.addEventListener('force-menu-refresh', handleForceMenuRefresh);
        
        return () => {
            window.removeEventListener('sidebar-refresh', handleSidebarRefresh);
            window.removeEventListener('force-menu-refresh', handleForceMenuRefresh);
        };
