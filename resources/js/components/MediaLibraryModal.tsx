...
export default function MediaLibraryModal({ 
  ...
  const handleFileUpload = async (files: FileList) => {
    ...
    try {
      ...
      try {
        ...
        result = { message: rawResponse || 'Unexpected response from server.' };
      }
      ...
      if (response.ok) {
        ...
        if (result.errors) {
          ...
          toast.error('Upload failed with no details returned. Please check server logs.');
        }
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[85vh] overflow-hidden" aria-describedby="media-library-description">
        ...
        <DialogDescription id="media-library-description" className="sr-only">
          Manage files, upload new media, and select assets for your card content.
        </DialogDescription>
